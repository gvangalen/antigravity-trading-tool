...
async def add_user_market_indicator(
    ...
    try:
        ...
        return await service.add_user_market_indicator(int(user_id), raw_name, value)
...
async def list_user_market_indicators(
    ...
    try:
        user_id = current_user["id"]
        service = MarketDataService(db)
        return await service.list_user_market_indicators(int(user_id), limit)
    except Exception as e:
...
async def delete_market_indicator(
    ...
    try:
        service = MarketDataService(db)
        return await service.delete_user_market_indicator(name, int(current_user["id"]))
    except HTTPException:
        raise
    except Exception as e:
...
async def get_market_day_data(
    ...
    try:
        service = MarketDataService(db)
        return await service.get_market_day_data(int(current_user["id"]))
    except Exception as e:
...
@router.get("/market_data/btc/latest", response_model=MarketDataResponse)
async def get_latest_btc_price(db: AsyncSession = Depends(get_db)):
    try:
        service = MarketDataService(db)
        result = await service.get_latest_btc_price()
        if not result:
        ...
        logger.error(f"❌ [latest_btc] Error: {e}")
        raise HTTPException(500, "Kon laatste BTC_prijs niet ophalen")
...
async def fetch_interpreted_data(
    ...
    try:
        service = MarketDataService(db)
        return await service.get_interpreted_data(int(current_user["id"]))
    except HTTPException:
        raise
    except Exception as e:
...
async def get_market_data_7d(db: AsyncSession = Depends(get_db)):
    try:
        service = MarketDataService(db)
        return await service.get_market_data_7d()
    except Exception as e:
...
async def get_market_forward_returns(db: AsyncSession = Depends(get_db)):
    try:
        service = MarketDataService(db)
        return await service.get_market_forward_returns()
    except Exception as e:
...
async def get_week_returns(db: AsyncSession = Depends(get_db)):
    try:
        service = MarketDataService(db)
        return await service.get_forward_returns_aggregated('7d')
    except Exception as e:
...
async def get_month_returns(db: AsyncSession = Depends(get_db)):
    try:
        service = MarketDataService(db)
        return await service.get_forward_returns_aggregated('30d')
    except Exception as e:
...
async def get_quarter_returns(db: AsyncSession = Depends(get_db)):
    try:
        service = MarketDataService(db)
        return await service.get_forward_returns_aggregated('90d')
    except Exception as e:
...
async def get_year_returns(db: AsyncSession = Depends(get_db)):
    try:
        service = MarketDataService(db)
        return await service.get_forward_returns_aggregated('365d')
    except Exception as e:
