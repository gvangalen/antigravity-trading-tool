...
def test_finn_v2_flags_default_to_disabled(monkeypatch):
    for name in (
        "FINN_V2_ENABLED",
...
def test_finn_v2_shadow_mode_requires_safe_readonly_config(monkeypatch):
    ...
    assert service.resolve_mode(11) == "shadow"
    assert service.allows_transport("chat") is True
    assert service.allows_transport("stream") is True


def test_finn_v2_canary_visible_mode_parses_user_ids(monkeypatch):
    monkeypatch.setenv("FINN_V2_ENABLED", "true")
    ...
    assert service.resolve_mode(8) == "visible_readonly"
    assert service.resolve_mode(9) == "disabled"
