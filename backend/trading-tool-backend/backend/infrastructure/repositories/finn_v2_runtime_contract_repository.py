...
class FinnV2RuntimeContractRepository(FinnV2RepositoryTransactionMixin):
    ...
    async def get_latest_action_result_for_user(
        self,
        *,
        user_id: int,
        exclude_run_id: Optional[str] = None,
    ) -> Optional[FinnV2RuntimeContract]:
        """Return the newest successful, owner-bound execution result.

        An executed action is durable user-owned lineage, not conversation
        JSON.  New conversations may therefore continue an object workflow,
        while the bounded scan prevents an unqualified historical fallback.
        Callers must still derive only contract-required slots from the result.
        """
        conditions = [FinnV2RuntimeContract.user_id == user_id]
        if exclude_run_id:
            conditions.append(FinnV2RuntimeContract.run_id != exclude_run_id)
        result = await self.session.execute(
            select(FinnV2RuntimeContract)
    ...
    async def get_recent_action_results_for_user(
        self,
        *,
        user_id: int,
        exclude_run_id: Optional[str] = None,
    ) -> dict[str, dict]:
        """Return the newest live execution result per entity type for one owner.

        This is not a generic implicit target. Consumers may use it only for
        an explicit demonstrative continuation such as ``deze setup`` after a
        browser/session boundary. Every result remains owner-scoped and typed.
        """
        conditions = [FinnV2RuntimeContract.user_id == user_id]
        if exclude_run_id:
            conditions.append(FinnV2RuntimeContract.run_id != exclude_run_id)
    ...
    async def record_guided_draft(self, *, run_id: str, guided_state: Dict[str, Any]) -> FinnV2RuntimeContract:
        """Persist any registry-backed guided create flow on its run contract."""
        row = await self._required_for_update(run_id)
