...
stop_local_processes() {
  # A prior interrupted local run can leave only these exact local processes.
  # Never match a production queue or an arbitrary uvicorn process.
  local pid command
  for pid in $(pgrep -f 'local-finn-finn_interactive|backend.main:app --host 127.0.0.1 --port 18000' || true); do
    command="$(ps -p "$pid" -o command= 2>/dev/null || true)"
    case "$command" in
      *local-finn-finn_interactive*|*'backend.main:app --host 127.0.0.1 --port 18000'*)
        kill "$pid" 2>/dev/null || true ;;
    esac
  done
  for _ in $(seq 1 20); do
    pgrep -f 'local-finn-finn_interactive|backend.main:app --host 127.0.0.1 --port 18000' >/dev/null || break
    sleep 0.1
  done
}
spawn_detached() {
...
case "${1:-}" in
  start)
    require_env; load_env; stop_local_processes; docker compose -f "$ROOT/docker-compose.finn-local.yml" up -d --wait
    ...
    for _ in $(seq 1 30); do
      if curl -fsS http://127.0.0.1:18000/api/health >/dev/null 2>&1; then "$0" health; exit 0; fi
      sleep 1
    done
  ...
  selector-eval)
    require_env; load_env
    shift
    (cd "$BACKEND" && FINN_V2_REAL_SELECTOR_EVAL=1 PYTHONPATH=. python3 backend/scripts/run_finn_v2_selector_eval.py "$@") ;;
  stop) stop_local_processes; docker compose -f "$ROOT/docker-compose.finn-local.yml" down ;;
  reset) "$0" stop; docker compose -f "$ROOT/docker-compose.finn-local.yml" down -v; find "$PID_DIR" -type f -maxdepth 1 -delete 2>/dev/null || true; rmdir "$PID_DIR" 2>/dev/null || true ;;
  *) echo "usage: $0 {start|health|logs|matrix|selector-eval|stop|reset}" >&2; exit 2 ;;
esac
