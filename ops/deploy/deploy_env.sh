...
REMOTE_LAST_GOOD="$(
  ssh "${SSH_ARGS[@]}" "ubuntu@$SERVER_IP" "
    if test -r ${REMOTE_DIR}/${DEPLOY_STATE_DIR}/LAST_GOOD_COMMIT; then
      cat ${REMOTE_DIR}/${DEPLOY_STATE_DIR}/LAST_GOOD_COMMIT
    elif sudo -n test -f ${CANONICAL_DEPLOY_STATE_DIR}/LAST_GOOD_COMMIT; then
      sudo -n cat ${CANONICAL_DEPLOY_STATE_DIR}/LAST_GOOD_COMMIT
    fi
  " 2>/dev/null | tail -n 1
)"
...
if ! printf '%s\n' "$DEPLOY_GIT_TOKEN" | timeout --foreground "${REMOTE_DEPLOY_COMMAND_TIMEOUT_SECONDS}s" ssh "${SSH_ARGS[@]}" "ubuntu@$SERVER_IP" "
  ...
  export PM2_DEPLOY_MODE='$PM2_DEPLOY_MODE'
  export BACKEND_APP='$BACKEND_APP'
  export CORE_PM2_APPS='$CORE_PM2_APPS'
  export AUX_PM2_APPS='$AUX_PM2_APPS'
  export EXPECTED_PM2_APPS='$EXPECTED_PM2_APPS'
  export MIGRATION_COMMAND_TIMEOUT_SECONDS=$MIGRATION_COMMAND_TIMEOUT_SECONDS
  export MIN_DEPLOY_AVAILABLE_MEMORY_KB=$MIN_DEPLOY_AVAILABLE_MEMORY_KB
  export DEPLOY_MEMORY_HEADROOM_ATTEMPTS=$DEPLOY_MEMORY_HEADROOM_ATTEMPTS
  export DEEP_HEALTH_ATTEMPTS=$DEEP_HEALTH_ATTEMPTS
  export DEEP_HEALTH_RETRY_DELAY_SECONDS=$DEEP_HEALTH_RETRY_DELAY_SECONDS
  ...
  # The PM2 ecosystem parser may read the established staging dotenv as data.
  # Never source it as shell code; valid dotenv values can contain shell
  # metacharacters. Keep it private while staging migrates to the central env.
  if [ "$ENVIRONMENT" = "staging" ] && [ -f "\$LEGACY_STAGING_ENV" ]; then
    chmod 600 "\$LEGACY_STAGING_ENV"
  fi
  if [ -f "\$ENV_FILE" ]; then
    chmod 600 "\$ENV_FILE"
    set -o allexport
