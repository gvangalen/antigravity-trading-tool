...
export default function AdminUsersDashboard() {
  ...
  if (loading) return (
    <div className="p-10 flex flex-col items-center justify-center min-h-[60vh]">
      ...
      <p className="text-slate-400 font-black uppercase tracking-widest text-[10px] italic">Gebruikers laden...</p>
    </div>
  );
        ...
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-3xl font-black text-slate-900 tracking-tight italic">
              Gebruikers <span className="text-blue-600">en toegang</span>
            </h1>
          </div>
          <p className="text-slate-500 font-medium max-w-2xl text-sm">
            Beheer toegang, AI-limieten en gebruiksactiviteit van alle geregistreerde accounts.
          </p>
        </div>
      ...
      <div className="bg-white border border-slate-100 rounded-[32px] shadow-sm overflow-hidden">
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-slate-50 bg-slate-50/30">
              <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Gebruiker</th>
              <th className="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Plan en status</th>
              <th className="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">AI-limiet</th>
              <th className="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Activiteit</th>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest flex items-center gap-1.5 mt-0.5">
                        {user.first_name ? `${user.first_name} ${user.last_name || ""}` : "Onbekende gebruiker"}
                      </p>
                    </div>
                  </div>
                </td>
                ...
                <td className="px-6 py-6 font-bold">
                  <div>
                    <div className="flex justify-between items-center mb-2">
                       <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest italic">Daglimiet</span>
                ...
                <td className="px-6 py-6 font-bold">
                   <div className="space-y-1.5">
                      <div className="flex items-center gap-2 text-[10px] font-black text-slate-500 uppercase tracking-widest italic">
                        ...
                        Laatst gezien: <span className="text-slate-900">{user.last_login_at ? new Date(user.last_login_at).toLocaleDateString() : "Nooit"}</span>
                      </div>
                      <div className="flex items-center gap-2 text-[10px] font-black text-slate-500 uppercase tracking-widest italic">
                        <Calendar size={12} className="text-slate-300" />
                        Aangemaakt: <span className="text-slate-900">{user.created_at ? new Date(user.created_at).toLocaleDateString() : "Onbekend"}</span>
                      </div>
                      <div className="flex items-center gap-2 text-[10px] font-black text-slate-500 uppercase tracking-widest italic">
                        <Zap size={12} className="text-slate-300" />
                        AI-activiteit: <span className="text-slate-900">{user.last_ai_activity_at ? new Date(user.last_ai_activity_at).toLocaleDateString() : "Geen gebruik"}</span>
                      </div>
                   </div>
                </td>
        ...
        {filteredUsers.length === 0 && (
          <div className="p-20 text-center">
             ...
             <p className="text-slate-400 font-black uppercase tracking-widest text-xs italic">Geen gebruikers gevonden</p>
          </div>
        )}
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-fade-in">
          <div className="bg-white w-full max-w-lg rounded-[32px] shadow-2xl overflow-hidden animate-slide-up border border-slate-100">
            <div className="p-8 border-b border-slate-50 flex justify-between items-start">
               <div>
                 <h2 className="text-xl font-black text-slate-900 italic tracking-tight mb-1">Gebruiker aanpassen</h2>
            ...
            <div className="p-8 space-y-8">
               {/* PLAN SELECT */}
               <div>
                  <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-4 block italic">Plan</label>
               ...
               {/* QUOTA */}
               <div>
                  <label className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-4 block italic">Dagelijkse limiet</label>
                  <div className="flex items-center gap-4">
                    <input 
                      type="range" 
                      min="10" 
               ...
               <div className="flex items-center justify-between p-6 bg-slate-50 rounded-[24px]">
                  <div>
                    <h4 className="text-[11px] font-black text-slate-900 uppercase tracking-widest italic mb-1">Toegang</h4>
                    <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Zet account aan of uit</p>
                  </div>
                  <button 
                  ...
                  >
                    {editingUser.is_active ? 'ACTIEF' : 'GEBLOKKEERD'}
                  </button>
               </div>
            </div>
