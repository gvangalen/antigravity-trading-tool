...
export default function AgentInsightPanel({ category, className = "" }) {
  ...
  if (!insight) {
    return (
      <div className={`card card-p flex items-center justify-center min-h-[200px] ${className}`}>
        <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest text-slate-300">
          Geen gegevens
        </p>
      </div>
    );
    ...
    <div className={`card ${className}`}>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h3 className="text-sm font-semibold text-slate-900 leading-none">Analyse</h3>
              <span className="status status-active">Actief</span>
            </div>
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">Marktsentiment</p>
          </div>
        </div>
      </div>
      ...
      <div className="card-p space-y-8">
        ...
        <div className="flex flex-wrap items-center gap-4">
           <div className="flex flex-col p-5 rounded-2xl bg-white border-2 border-slate-100 min-w-[140px] flex-1">
              <span className="metric-label text-blue-600">Trend</span>
              <div className="flex items-center gap-2">
                 {trendIcon}
                 <span className="text-sm font-black text-slate-800 uppercase tracking-tight">{trend || "Neutraal"}</span>
              </div>
           </div>
           ...
           <div className="flex flex-col p-5 rounded-2xl bg-white border-2 border-slate-100 min-w-[140px] flex-1">
              ...
              <div className="flex items-center gap-2">
                 ...
                 <span className="text-sm font-black text-slate-800 uppercase tracking-tight">{bias || "Geen"}</span>
              </div>
           </div>
           ...
           <div className="flex flex-col p-5 rounded-2xl bg-white border-2 border-slate-100 min-w-[140px] flex-1">
              ...
              <div className="flex items-center gap-2">
                 ...
                 <span className="text-sm font-black text-slate-800 uppercase tracking-tight">{risk || "Laag"}</span>
              </div>
           </div>
        </div>
        ...
        <div className="space-y-4">
           ...
           <div className="bg-gradient-to-br from-white to-slate-50 border-2 border-slate-100 p-8 rounded-2xl relative overflow-hidden">
              ...
              <p className="text-sm font-medium text-slate-800 leading-relaxed max-w-4xl tracking-tight italic">
                {summary}
              </p>
           </div>
        </div>
        ...
        {cleanSignals.length > 0 && (
          <div className="space-y-3">
             <div className="text-[11px] font-bold text-slate-400 uppercase tracking-widest">Signalen</div>
             <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                {cleanSignals.map((s, i) => (
                  <div key={i} className="flex items-center gap-3 p-4 bg-white border border-slate-100 rounded-lg shadow-sm hover:border-blue-200 hover:bg-blue-50 transition-all">
                     <div className="w-1.5 h-1.5 rounded-full bg-blue-500 shrink-0" />
                     <div className="text-[10px] font-semibold text-slate-600 uppercase tracking-tight leading-tight">
                        {s}
                     </div>
                  </div>
                ))}
        ...
        <footer className="pt-6 border-t border-slate-100 flex items-center justify-between opacity-50">
           <div className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">Controle voltooid</div>
           {lastUpdate && (
             <div className="text-[9px] font-bold text-slate-400 uppercase tracking-[0.1em]">
