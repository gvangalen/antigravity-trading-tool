...
export default function TechnicalTerminalHUD({ score, bias, trend, risk, loading = false }) {
  ...
  const copy = t?.pages?.technical?.hud || {};
  if (loading) {
    return <HUDSkeleton />;
  }
  ...
  const biasLabel = {
    bullish: copy.positiveBias,
    bearish: copy.negativeBias,
    neutral: copy.neutral,
    ranging: copy.sidewaysBias,
    stable: copy.stableBias,
  }[String(bias || "").toLowerCase()] || bias || copy.stableBias;
  const trendLabel = {
    bullish: copy.upwardTrend,
    bearish: copy.downwardTrend,
    ranging: copy.sidewaysTrend,
    range: copy.sidewaysTrend,
    neutral: copy.neutral,
    stable: copy.stableBias,
  }[String(trend || "").toLowerCase()] || trend || copy.stableBias;
  const riskLabel = {
    high: copy.riskHigh,
    medium: copy.riskMedium,
    low: copy.riskLow,
    stable: copy.stableBias,
  }[String(risk || "").toLowerCase()] || risk || copy.stableBias;
      ...
      <div className="lg:col-span-2 bg-card rounded-[2.5rem] border border-[var(--color-border)] p-6 sm:p-10 shadow-[0_8px_30px_rgb(0,0,0,0.04)] flex flex-col justify-between relative overflow-hidden group transition-all hover:shadow-[0_20px_50px_rgba(0,0,0,0.08)]">
         ...
         <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-10 relative z-10">
            <div className="flex items-center gap-4">
               ...
               <div>
                  <div className="text-[10px] sm:text-[11px] font-bold text-secondary/60 uppercase tracking-[0.2em] mb-0.5">{copy.visualEyebrow}</div>
                  <div className="text-xl sm:text-2xl font-black text-foreground tracking-tight uppercase leading-none">{copy.contextTitle}</div>
               </div>
            </div>
         ...
         <div className="space-y-8 relative z-10">
            <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
               ...
               <div className="text-left sm:text-right">
                  <div className="text-[10px] font-black text-secondary/40 uppercase tracking-[0.2em] mb-1 sm:mb-2 leading-none">{copy.directionLabel}</div>
      ...
      <div className="bg-card rounded-[2.5rem] border border-[var(--color-border)] p-6 sm:p-10 flex flex-col justify-between shadow-[0_8px_30px_rgb(0,0,0,0.04)] relative overflow-hidden group h-full transition-all hover:shadow-[0_20px_50px_rgba(0,0,0,0.08)]">
         ...
         <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8 relative z-10">
            <div className="flex items-center gap-4">
               ...
               <div className="text-[10px] sm:text-[11px] font-bold uppercase tracking-[0.2em] text-secondary/60 leading-none">{copy.signalFlow}</div>
            </div>
         </div>

         <div className="relative z-10 space-y-3">
            <div className="text-[10px] sm:text-[11px] font-black text-blue-500 uppercase tracking-[0.25em]">{copy.dominantTrend}</div>
            ...
            <div className="py-3 px-4 rounded-xl bg-slate-50 dark:bg-slate-900/50 border border-[var(--color-border-subtle)]">
               <p className="text-[9px] sm:text-[10px] text-secondary/70 leading-relaxed font-bold uppercase tracking-wider">
                  {copy.trendSummary.replace("{trend}", String(trendLabel).toLowerCase())}
               </p>
            </div>
         </div>
