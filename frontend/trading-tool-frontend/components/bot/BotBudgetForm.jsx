    ...
    <form ref={formRef} onSubmit={handleSubmit} className="space-y-8 p-1">
      ...
      {riskAcknowledgement ? (
        <div className="rounded-2xl border border-amber-300 bg-amber-50 px-4 py-4 text-sm text-amber-950">
          <p className="font-black">{copy.riskAcknowledgementTitle || "Bevestig de risicowijziging"}</p>
          <p className="mt-1 font-medium leading-relaxed">
            {riskAcknowledgement.message || copy.riskAcknowledgementBody || "Deze wijziging verhoogt een risicolimiet. Bevestig alleen als je dit bewust wilt opslaan."}
          </p>
          <div className="mt-4 flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
            ...
            >
              {copy.riskAcknowledgementCancel || "Wijziging aanpassen"}
            </button>
            <button
              type="button"
              disabled={loading}
            ...
            >
              {copy.riskAcknowledgementConfirm || "Ik begrijp dit en sla op"}
            </button>
          </div>
        </div>
      ) : null}
