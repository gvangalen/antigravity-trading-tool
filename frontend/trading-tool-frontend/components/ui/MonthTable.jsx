...
export default function MonthTable({ data = [], onRemove }) {
  ...
  const groups = groupByMonth(data, locale);
  ...
  return (
    <div className="space-y-10 w-full">
      {groups.map((group, gIdx) => (
        <div key={gIdx} className="bg-card border border-slate-200 rounded-[2rem] shadow-sm overflow-hidden group/table">
          {/* TERMINAL HEADER */}
          <div className="px-8 py-5 border-b border-slate-100 bg-slate-50/50">
            <div className="flex items-center gap-3">
              ...
              <div className="text-[10px] font-black text-muted uppercase tracking-widest leading-none">
                 {copy.monthlyOverview}: {group.label.toUpperCase()}
              </div>
            </div>
          </div>
...
function groupByMonth(items, locale) {
  ...
  items.forEach((item) => {
    ...
    const monthName = new Intl.DateTimeFormat(getIntlLocale(locale), { month: "long" }).format(date);
